...
public class _06_VariableEx {
	public static void main(String[] arg){
		...
		// 1. 자동 형변환  : 묵시적 형변환 => 형변환 생략 가능
		double dval = 20;
		System.out.println("dval : " + dval); // dva : 20.0
		
		float fval = 20.5f +10; // 20.5f + 10.0f;
		System.out.println("fval : " + fval); // fval : 30.5
		
		double dval2 = 10.5 + 30; // 10.5 + 30.0
		System.out.println("fval : " + dval2); // dval2 : 40.5
		
		//2. 강제 형변환 : 명시적 형변환 => 형변환 생략 불가이며, 생략시 컴파일 에러가 발생하므로 반드시 형변환해야 한다.
		
		int ival = (int) 10.5; // 명시적형변환
		System.out.println("ival : " + ival);
		
		int num1= (int)(10.5 +30); // ()가 우선순위, 큰 타입으로 통일(10.5 + 30.0)
		System.out.println("num1 : " + num1); // num1 : 40
		
		int num2 = (int) 10.5 + 30; // 10 + 30 = 40
		System.out.println("num2 : " + num2); // num2 : 40
		
		/*
		 * 형변환-2
		 * 
		 * 3. int(4바이트)보다 작은 자료형(byte, short)은 int로 형변환 후 계산
		 * 예) byte + short => int + int
		 * 
		 * 4. 두개의 피연사자중 표현 범위가 큰쪽에 맞춰서 형변환후 계산
		 *
		 */
		...
		ival2 = 5/4; // int / int => int형임, 원래 결과는 1.25이나 int로 대입되면서 1로 1바뀜
		System.out.println("ival2 : " + ival2); // ival2 : 1
		
		dval3 = 5/4; // int / int => int형임, 원래 결과는 1.25이나 int형 1, double형으로 바뀜
		System.out.println("dval3 : " + dval3);
		
		ival3 = (int) (2.5 + 3.2); // (int) (5.7)
		System.out.println("dval3 : " + ival3); // ival3 : 5
...
//		long lval2 = 20000000000L;
		...
		// 연산 후 대입 => long형으로 대입하기 전부터 연산결과가 int형 범위초과(오버플로우 : 잘못된 데이터카 들어온다.)
		...
		//4. 두개의 피연사자중 표현 범위가 큰쪽에 맞춰서 형변환후 계산
		long num6 = (long) ival4 * ival5; // long * int이므로 long형으로 자동형변환
