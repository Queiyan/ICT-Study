...
public class _02_ArrayList {
     ...
     */
     ...
     // 클래스명<데이터타입클래스> 참조변수 = new 클래스명<데이터타입클래스>();
		...
		while(itr.hasNext()) { 			// ArrayList의 값이 존재하는지 체크
			String food = itr.next();	// 존재하는 값을 읽어서 변수에 대입
			System.out.println(food);
		}
		
		Iterator itr2 = list.iterator();
		while(itr2.hasNext()) { 		// ArrayList의 값이 존재하는지 체크
			Object food = itr2.next();	// iterator 타입 생략시 Object(=최상위클래스) 형으로 반환하므로 다운캐스팅
			System.out.println(food);
		}
